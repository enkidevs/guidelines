...
var INDEX_FILENAME = 'search_index.json';
 ...
 */
function load(json) {
    return { type: TYPES.LOAD, json: json };
}
...
function fetch() {
    return function (dispatch, getState) {
        ...
        var filePath = file.relative(INDEX_FILENAME);

        if (idx) {
            return GitBook.Promise.resolve();
        }

        return GitBook.Promise.resolve().then(function () {
            return window.fetch(filePath);
        }).then(function (response) {
            return response.json();
        }).then(function (json) {
            return dispatch(load(json));
        });
    };
}
...
},{"gitbook-core":"gitbook-core"}],3:[function(require,module,exports){
'use strict';

var GitBook = require('gitbook-core');
var reduce = require('./reducer');
...
function searchHandler(query, dispatch, getState) {
    ...
    // Execute the search
    .then(function () {
        var _getState$lunr = getState().lunr,
            idx = _getState$lunr.idx,
            store = _getState$lunr.store;

        var results = idx.search(query);

        return results.map(function (_ref) {
            var ref = _ref.ref;
            return store.get(ref).toJS();
        });
    });
}
...
},{"./actions":2,"./reducer":4,"gitbook-core":"gitbook-core"}],4:[function(require,module,exports){
'use strict';
...
var LunrState = Record({
    idx: null,
    store: {}
});

module.exports = GitBook.createReducer('lunr', function (state, action) {
    state = state || LunrState();

    switch (action.type) {

        case TYPES.LOAD:
            return state.set('idx', lunr.Index.load(action.json.index)).merge({
...
},{"./actions":2,"gitbook-core":"gitbook-core","lunr":1}]},{},[3])(3)
});